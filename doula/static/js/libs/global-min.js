...
SM.String={getPercent:function(val,tot,decimalPoints){var top,bottom;if(tot===0)return"N/A";if(decimalPoints===undefined){decimalPonts=1;}
top=Math.pow(10,decimalPoints+2)*(val/tot);bottom=Math.pow(10,decimalPoints);return(Math.round(top)/bottom)+"%";},sanitize:function(str){var div=document.createElement("div");if(!str)return"";div.innerHTML=str;return div.textContent||div.innerText;},truncate:function(str,max_length){if(!str)return"";if(!max_length)return str;if(str.length&&str.length>max_length){return str.substring(0,max_length)+"...";}
return str;}};
